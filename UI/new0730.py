...
class FileTransferThread(QThread):
        ...
        try:
            ...
            if self.operation == 'upload':
                ...
                if os.path.isdir(self.local_path):
                    ...
                    # Upload using SFTP
                    sftp.put(tar_path, self.remote_path)
                    ...
                    # Clean up
                    try:
                        sftp.remove(self.remote_path)  # Remove tar file
                    except:
                        pass
                    os.unlink(tar_path)  # Remove local temp file
                else:
                    self.progress_update.emit("Uploading file...")
    ...
    def _create_remote_dirs(self, sftp, remote_path):
...
class ServerConnectionModule(QWidget):
    ...
    def connect_to_server(self):
        ...
        # Use values from read-only input fields (hardcoded values)
        hostname = self.ip_edit.text()
        username = self.username_edit.text()
        password = self.password_edit.text()
        
        self.connection_thread = SSHConnectionThread(hostname, username, password)
...
class ModelTrainingModule(QWidget):
    ...
    def setup_ui(self):
        ...
        # Result display areas (horizontal layout)
        results_display_layout = QHBoxLayout()
        
        self.hazemap_label = QLabel("Hazemap Visualization")
        self.hazemap_label.setMinimumHeight(200)
        self.hazemap_label.setMinimumWidth(200)
        self.hazemap_label.setStyleSheet("border: 1px solid #ccc; background-color: #f9f9f9;")
        self.hazemap_label.setAlignment(Qt.AlignCenter)
        
        self.ground_truth_label = QLabel("Ground Truth Comparison")
        self.ground_truth_label.setMinimumHeight(200)
        self.ground_truth_label.setMinimumWidth(200)
        ...
        self.postprocess_label.setMinimumHeight(200)
        self.postprocess_label.setMinimumWidth(200)
        self.postprocess_label.setStyleSheet("border: 1px solid #ccc; background-color: #f9f9f9;")
        self.postprocess_label.setAlignment(Qt.AlignCenter)
        
        results_display_layout.addWidget(self.hazemap_label)
        results_display_layout.addWidget(self.ground_truth_label)
        results_display_layout.addWidget(self.postprocess_label)
...
class ModelTestingModule(QWidget):
    ...
    def setup_ui(self):
        ...
        self.hazemap_result_label.setMinimumHeight(200)
        self.hazemap_result_label.setMinimumWidth(300)
        self.hazemap_result_label.setStyleSheet("border: 1px solid #ccc; background-color: #f9f9f9;")
        self.hazemap_result_label.setAlignment(Qt.AlignCenter)
        
        self.postprocess_result_label = QLabel("Post-processing Results")
        self.postprocess_result_label.setMinimumHeight(200)
        self.postprocess_result_label.setMinimumWidth(300)
        self.postprocess_result_label.setStyleSheet("border: 1px solid #ccc; background-color: #f9f9f9;")
        self.postprocess_result_label.setAlignment(Qt.AlignCenter)
        
        results_display_layout.addWidget(self.hazemap_result_label)
        results_display_layout.addWidget(self.postprocess_result_label)
