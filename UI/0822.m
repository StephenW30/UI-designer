...
classdef WaferMapAnnotator < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.UIFigure.Name = 'Wafer Map Annotator (Optimized - Zoom Fixed)';
    ...
    methods (Access = private)
        ...
        function startupFcn(app)
            ...
            fprintf('\n=== Optimized Wafer Map Annotator (Zoom Fixed) ===\n');
            fprintf('Improvements:\n');
            fprintf('1. Fixed zoom state preservation during annotation\n');
            fprintf('2. Optimized drawing performance with fast overlays\n');
            fprintf('3. Improved mouse event handling\n');
            fprintf('4. Synchronized zoom between panels\n');
        end
    end
    ...
    methods (Access = private)
        ...
        function drawAtPosition(app, x, y)
            % Draw at specific position (修复版本 - 坐标转换一致性)
            if isempty(app.workingMap)
                return;
            end
            ...
            if xPixel >= 1 && xPixel <= w && yPixel >= 1 && yPixel <= h
                ...
                % **关键优化：在绘制过程中只更新overlay，不重绘base image**
                if app.IsDrawing
                    % 绘制过程中：快速添加overlay点，不完全重绘
    ...
    methods (Access = private)
        ...
        function updateHazemapDisplay(app)
            ...
            if isempty(app.hazemapImageHandle) || ~isvalid(app.hazemapImageHandle)
                ...
                % 只清除overlay，保留base image
                children = app.HazemapUIAxes.Children;
                % 删除除了base image以外的所有对象
